...
/// Maximum number of bets/raises per street before only call/fold (and all-in) remain.
/// `0` disables the cap. Set through [`set_max_raises`]; used to keep trees small on phones.
static MAX_RAISES: std::sync::atomic::AtomicI32 = std::sync::atomic::AtomicI32::new(0);

/// Sets the global cap on the number of bets/raises per street (`0` = unlimited).
/// Must be called before the tree is built. When the cap is reached, the raise sizes are dropped
/// and only fold / call / all-in remain; `allin_after_cap` controls whether all-in stays.
pub fn set_max_raises(max_raises: i32) {
    MAX_RAISES.store(max_raises, std::sync::atomic::Ordering::Relaxed);
}
...
impl ActionTree {
    ...
    fn push_actions(&self, node: &mut ActionTreeNode, info: &BuildTreeInfo) {
            ...
            && info.oop_call_flag
        {
            ...
            let max_raises = MAX_RAISES.load(std::sync::atomic::Ordering::Relaxed);
            let capped = max_raises > 0 && info.num_bets >= max_raises;

            if !info.allin_flag && capped {
                // raise cap reached: only an all-in re-raise remains
                actions.push(Action::AllIn(max_amount));
            }
