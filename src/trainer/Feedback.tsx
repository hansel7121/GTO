...
export function DecisionFeedback({ decision, cls, onContinue, continueLabel }: { decision: TrainerDecision; cls: string; onContinue: () => void; continueLabel: string }) {
    ...
    <div className={`rounded-md border p-3 space-y-2 bg-[#1e1e1e] ${ok ? 'border-[#3f9e5a]' : 'border-[#c43c3c]'}`}>
      <div className="flex items-center justify-between gap-2">
        <div className={`font-bold uppercase tracking-wider text-sm ${ok ? 'text-[#4fc06f]' : 'text-[#e04b4b]'}`}>
          {a.correct === undefined ? 'Not graded' : ok ? '✓ Correct' : '✗ Mistake'}
        </div>
        <div className="text-xs text-[#9a9a9a]">
          {cls} · {scenarioText(a.scenario)}
        </div>
      </div>
      ...
      {best ? <Mix analysis={a} /> : <div className="text-xs text-[#9a9a9a]">Off-model line (limping is not part of the 30bb strategy) — not graded.</div>}
...
function actionText(d: TrainerDecision) {
  const c = d.chosen
  if (!c) return '—'
    ...
    <div className="space-y-1">
      {analysis.options.map((o, i) => (
        <div key={i} className="flex items-center gap-2 text-xs">
          <div className={`w-28 truncate ${i === analysis.chosenIndex ? 'text-white font-semibold' : 'text-[#bdbdbd]'}`}>{o.label}</div>
